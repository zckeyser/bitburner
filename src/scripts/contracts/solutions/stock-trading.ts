...
export async function main(ns: NS) {
    ...
    let bestTrade = analyzeTrades(ns, k, prices);
    logger.info(`The best available trade margin is: ${bestTrade}`);
}
 ...
 * @returns 
 */
export function analyzeTrades(ns: NS, maxTransactions: number, prices: number[]): number {
    const logger = new TermLogger(ns);
    if(prices.length < 2) {
        return 0;
    }
    if (maxTransactions > prices.length / 2) { // Is this valid if the input array hasn't been optimized?
        ...
        logger.info(`More transactions available than can be used. Maximum profit is ${sum}.`);
        return sum;
    }
    ...
    logger.info(`Maximum profit is ${profit}`);
    return profit;
}
